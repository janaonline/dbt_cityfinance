...
final_base AS (
    SELECT
        ...
        CASE
            ...
            WHEN COALESCE(l.has_error_value, FALSE) = TRUE THEN 'error'
        ...
        (
            COALESCE(l.secured_loans, 0) + COALESCE(l.unsecured_loans, 0)
        ) AS "Existing Debt Exposure",
        (
            COALESCE(l.total_revenue, 0) - COALESCE(l.revenue_expenditure, 0) + COALESCE(l.Interest_Finance_Charges, 0) + COALESCE(l.Depreciation_on_Fixed_Assets, 0)
    ...
    FROM
        ulb_year_base b
        LEFT JOIN standardization_logs l ON b.ulb_id :: TEXT = l.ulb_id
        AND (
            b.year_id :: TEXT = l.year_key
            OR b.financial_year :: TEXT = l.year_key
        )
    WHERE
        b.financial_year_start >= 2019
)
...
SELECT
    *,
    CASE
        ...
        AND COALESCE("Existing Debt Exposure", 0) >= 0 THEN GREATEST(
